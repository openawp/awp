...
void Model::creatHalfEdge() {
	...
	for (int i = 0; i < cntFaces; i++) {
		for (int j = 0; j < 3; j++) {
			...
			halfEdges[sizeOfHalf++] = newHalfEdge;//halfEdges.push_back(newHalfEdge);
...
void Model::createPoint2dByAng(int eid, float ang, float a, float b, float c) {
	...
	if (fabs(angOnEdge[eid].y) < EPS) {
		puts("hahah");
		printf("%lf %lf %lf %lf\n", a, b, c, angOnEdge[eid].x);
	}
}

void Model::findAllInfoOnVers() {
	int ss = 0, i;
	//printf("%d\n", vertexs.size());
	...
	//	occupy.push_back(-1);
		...
		do {
			...
			//edgeOfVer[ss++] = teid;//Eid.push_back(teid);
		...
		//ss = max((int)Eid.size(), ss);
		//edgeOfVer.push_back(Eid);
		if (ang > 2 * PI) isNieght[i] = true;
		else isNieght[i] = false;
	}
	//sizeOfE[i] = ss;
	//printf("------------%d\n", ss);
}
...
vector<int> Model::findAllAdjVer(int vid, int eid) {
	vector<int> adjVers;
	adjVers.clear();
	int nextId, tid = eid;
	while (1) {
	...
	while (1) {
		int idOfNextEdge = Model::idOfNextHalfEdge(tid);
		adjVers.push_back(halfEdges[idOfNextEdge].idOfStartPoint);
		nextId = halfEdges[idOfPreHalfEdge(Model::idOfNextHalfEdge(idOfNextEdge))].idOfOppositeEdge;
		if (nextId == -1) {
			int idOfOppositeEdge = halfEdges[eid].idOfOppositeEdge;
			while (idOfOppositeEdge != -1) {
				nextId = Model::idOfNextHalfEdge(idOfOppositeEdge);
				adjVers.push_back(halfEdges[Model::idOfPreHalfEdge(idOfOppositeEdge)].idOfStartPoint);
...
void Model::ReadObjFile(char * filename)
{
...
void Model::ReadFile(char * filename)
{
...
void Model::ReadMFile(char *filename) {
...
void Model::ReadAsciiPly(char * filename)
{
	ifstream in(filename, ios_base::in | ios_base::binary);
	if (in.fail())
	{
...
void Model::ReadBinaryPly(char * filename, const char* format)
{
	ifstream in(filename, ios_base::in | ios_base::binary);
	if (in.fail())
	{
...
void Model::ReadUntilNextLine(ifstream& in) const
{
	if (in.eof())
		return;
	char ch;
...
void Model::SwapOrder(char *buf, int sz)
{
...
void Model::ReadIntoWord(ifstream &in, string& word) const
{
	word.clear();
	char ch;
